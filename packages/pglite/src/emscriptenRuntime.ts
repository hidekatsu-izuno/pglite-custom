...
type WasmFunction = readonly [fn: CallableFunction, sig: string]

async function instantiateNodeWasm(
  ...
  try {
    const binary = wasmBinary ?? new Uint8Array(fs.readFileSync(wasmBinaryFile))
...
const bigintToI53Checked = (num: bigint) =>
  num < -I53_LIMIT || num > I53_LIMIT ? NaN : Number(num)

const FS_FLAG_MODES: Record<string, number> = {
  r: 0,
  'r+': 2,
  w: 512 | 64 | 1,
  'w+': 512 | 64 | 2,
...
const FS_getMode = (canRead: boolean, canWrite: boolean) => {
  let mode = 0
...
const HEAP_MAX = 2147483648

const createMemoryViews = (wasmMemory: WebAssembly.Memory) => {
...
export const createMemoryViewManager = (wasmMemory: WebAssembly.Memory) => {
  const views = createMemoryViews(wasmMemory)

  return Object.assign(views, {
    refresh() {
      Object.assign(views, createMemoryViews(wasmMemory))
      return views
    },
  })
}

type ExceptionMemoryViews = Pick<
  ReturnType<typeof createMemoryViews>,
  'HEAP8' | 'HEAPU32'
>
...
class ExceptionInfo {
  ...
  constructor(
    private readonly memoryViews: ExceptionMemoryViews,
    readonly excPtr: number,
  ) {
    this.ptr = excPtr - 24
  }

  set_type(type: number) {
    this.memoryViews.HEAPU32[(this.ptr + 4) >> 2] = type
  }

  get_type() {
    return this.memoryViews.HEAPU32[(this.ptr + 4) >> 2]
  }

  set_destructor(destructor: number) {
    this.memoryViews.HEAPU32[(this.ptr + 8) >> 2] = destructor
  }

  get_destructor() {
    return this.memoryViews.HEAPU32[(this.ptr + 8) >> 2]
  }

  set_caught(caught: boolean) {
    this.memoryViews.HEAP8[this.ptr + 12] = caught ? 1 : 0
  }

  get_caught() {
    return this.memoryViews.HEAP8[this.ptr + 12] != 0
  }

  set_rethrown(rethrown: boolean) {
    this.memoryViews.HEAP8[this.ptr + 13] = rethrown ? 1 : 0
  }

  get_rethrown() {
    return this.memoryViews.HEAP8[this.ptr + 13] != 0
  }
  ...
  set_adjusted_ptr(adjustedPtr: number) {
    this.memoryViews.HEAPU32[(this.ptr + 16) >> 2] = adjustedPtr
  }

  get_adjusted_ptr() {
    return this.memoryViews.HEAPU32[(this.ptr + 16) >> 2]
  }
}

const callRuntimeCallbacks = (
...
const createRunDependencyManager = (module: {
...
const uleb128Encode = (n: number, target: number[]) => {
  if (n < 128) {
    target.push(n)
  } else {
...
const sigToWasmTypes = (sig: string) => {
...
const generateFuncType = (sig: string, target: number[]) => {
...
const convertJsFunctionToWasm = (func: CallableFunction, sig: string) => {
...
const ydayFromDateCommon = (
  date: Date,
  leapDays: number[],
  regularDays: number[],
) => {
  const year = date.getFullYear()
  const monthDaysCumulative =
...
const createPathFS = (getCwd: () => string) => {
  const pathFS = {
    resolve: (...args: string[]) => {
      let resolvedPath = ''
...
/**
 * Internal configuration for an Emscripten module.  Keeping the generated
 * runtime behind this boundary lets the small public factories describe only
 * the differences between the postgres and initdb programs.
 */
export interface EmscriptenRuntimeOptions {
  wasmBinaryFile: URL
  wasmInput: 'binary' | 'module-or-binary'
  initialMemory: number
  heapBase: number
  tableSize: number
  filesystem: {
    initializeNodeFs: boolean
    mountPipeFs: boolean
    preloadPostgresPackage: boolean
    registerWasmPlugin: boolean
  }
  publicApi: 'postgres' | 'initdb'
}

export const createEmscriptenRuntime = async <TModule extends object>(
  emscriptenOpts: TModule,
  runtimeOptions: EmscriptenRuntimeOptions,
) => {
  const Module = emscriptenOpts as TModule & Record<string, any>
  const loadPostgresPackage = runtimeOptions.filesystem.preloadPostgresPackage
    ? preparePostgresPackage(Module['getPreloadedPackage']?.bind(Module))
    : undefined
  const thisProgram =
    Module['thisProgram'] ||
  ...
  const wasmBinary =
    runtimeOptions.wasmInput === 'module-or-binary'
      ? Module['wasmModule'] || Module['wasmBinary']
      : Module['wasmBinary']
  const wasmMemory =
    Module['wasmMemory'] ||
    new WebAssembly.Memory({
      initial:
        (Module['INITIAL_MEMORY'] || runtimeOptions.initialMemory) / 65536,
      maximum: 32768,
    })
  let ABORT = false
  let EXITSTATUS
  ...
  const memoryViews = createMemoryViewManager(wasmMemory)
  const updatePublicMemoryViews = () => {
    if (runtimeOptions.publicApi === 'postgres') {
      Object.assign(Module, {
        HEAP8: memoryViews.HEAP8,
        HEAPU8: memoryViews.HEAPU8,
      })
    }
  }
  updatePublicMemoryViews()
  ...
  function initRuntime() {
    ...
    TTY.init()
    if (runtimeOptions.filesystem.mountPipeFs) {
  ...
  const wasmBinaryFile = runtimeOptions.wasmBinaryFile
  ...
  let ___heap_base = runtimeOptions.heapBase
  ...
  const wasmTable = new WebAssembly.Table({
    initial: runtimeOptions.tableSize,
    element: 'anyfunc',
  })
  ...
  const updateGOT = (exports, replace) => {
    for (const symName in exports) {
      if (INTERNAL_SYMBOLS.has(symName) || symName.startsWith('__em_js__')) {
        continue
      }
  ...
  const createInvokeFunction =
    (sig) =>
    (ptr, ...args) => {
      const sp = _emscripten_stack_get_current()
      try {
  ...
  const resolveGlobalSymbol = (symName, direct = false) => {
    ...
    if (isSymbolDefined(symName)) {
      ...
      const signature = symName.split('_')[1]
      sym = createInvokeFunction(signature)
      registerWasmFunction(symName, [sym, signature])
    }
    return { sym, name: symName }
  }
  const UTF8ToString = (ptr, maxBytesToRead) =>
    ptr ? UTF8ArrayToString(memoryViews.HEAPU8, ptr, maxBytesToRead) : ''
  const loadWebAssemblyModule = (
    ...
    function loadModule() {
      const firstLoad = !handle || !memoryViews.HEAP8[handle + 8]
      let memoryBase
      let tableBase
      if (firstLoad) {
        ...
        if (handle) {
          memoryViews.HEAP8[handle + 8] = 1
          memoryViews.HEAPU32[(handle + 12) >> 2] = memoryBase
          memoryViews.HEAP32[(handle + 16) >> 2] = metadata.memorySize
          memoryViews.HEAPU32[(handle + 20) >> 2] = tableBase
          memoryViews.HEAP32[(handle + 24) >> 2] = metadata.tableSize
        }
      } else {
        memoryBase = memoryViews.HEAPU32[(handle + 12) >> 2]
        tableBase = memoryViews.HEAPU32[(handle + 20) >> 2]
      }
      ...
      function postInstantiation(module, instance) {
        ...
        if ('__start_em_asm' in moduleExports) {
          ...
          while (start < stop) {
            ...
            start = memoryViews.HEAPU8.indexOf(0, start) + 1
          }
        }
  ...
  function loadDynamicLibrary(
    ...
    function loadLibData() {
      if (handle) {
        const data = memoryViews.HEAPU32[(handle + 28) >> 2]
        const dataSize = memoryViews.HEAPU32[(handle + 32) >> 2]
        if (data && dataSize) {
          const libData = memoryViews.HEAP8.slice(data, data + dataSize)
  ...
  const reportUndefinedSymbols = () => {
    for (const [symName, entry] of Object.entries(GOT)) {
      if (entry.value == 0) {
        ...
        if (typeof value == 'function') {
          entry.value = addFunction(value, wasmFunctionSignatures.get(value))
        } else if (typeof value == 'number') {
          entry.value = value
        } else {
  ...
  function ___syscall_pipe(fdPtr) {
    try {
      ...
      memoryViews.HEAP32[fdPtr >> 2] = res.readable_fd
      memoryViews.HEAP32[(fdPtr + 4) >> 2] = res.writable_fd
      return 0
    } catch (e) {
  ...
  function ___syscall_readlinkat(dirfd, path, buf, bufsize) {
    try {
      ...
      const endChar = memoryViews.HEAP8[buf + len]
      stringToUTF8Array(ret, memoryViews.HEAPU8, buf, bufsize + 1)
      memoryViews.HEAP8[buf + len] = endChar
      return len
    } catch (e) {
      if (typeof FS == 'undefined' || !(e.name === 'ErrnoError')) throw e
      return -e.errno
  ...
  function ___syscall_statfs64(path, size, buf) {
    try {
      ...
      memoryViews.HEAP32[(buf + 4) >> 2] = stats.bsize
      memoryViews.HEAP32[(buf + 40) >> 2] = stats.bsize
      memoryViews.HEAP32[(buf + 8) >> 2] = stats.blocks
      memoryViews.HEAP32[(buf + 12) >> 2] = stats.bfree
      memoryViews.HEAP32[(buf + 16) >> 2] = stats.bavail
      memoryViews.HEAP32[(buf + 20) >> 2] = stats.files
      memoryViews.HEAP32[(buf + 24) >> 2] = stats.ffree
      memoryViews.HEAP32[(buf + 28) >> 2] = stats.fsid
      memoryViews.HEAP32[(buf + 44) >> 2] = stats.flags
      memoryViews.HEAP32[(buf + 36) >> 2] = stats.namelen
      return 0
    } catch (e) {
      if (typeof FS == 'undefined' || !(e.name === 'ErrnoError')) throw e
      return -e.errno
  ...
  const readI53FromI64 = (ptr) =>
    memoryViews.HEAPU32[ptr >> 2] +
    memoryViews.HEAP32[(ptr + 4) >> 2] * 4294967296
  function ___syscall_utimensat(dirfd, path, times, flags) {
    try {
      ...
      if (!times) {
        ...
        let nanoseconds = memoryViews.HEAP32[(times + 8) >> 2]
        ...
        nanoseconds = memoryViews.HEAP32[(times + 8) >> 2]
        if (nanoseconds == 1073741823) {
          mtime = now
        } else if (nanoseconds == 1073741822) {
          mtime = null
  ...
  const stringToUTF8OnStack = (str) => {
    ...
    const pointer = stackAlloc(size)
    stringToUTF8Array(str, memoryViews.HEAPU8, pointer, size)
    return pointer
  }
  ...
  const dlopenInternal = (handle, jsflags) => {
    ...
    const flags = memoryViews.HEAP32[(handle + 4) >> 2]
  ...
  const __dlsym_js = (handle, symbol, symbolIndex) => {
    ...
    if (typeof result == 'function') {
      ...
      if (addr) {
        ...
        result = addFunction(result, wasmFunctionSignatures.get(result))
        memoryViews.HEAPU32[symbolIndex >> 2] = newSymIndex
      }
    }
    return result
  }
  ...
  function __gmtime_js(time, tmPtr) {
    ...
    memoryViews.HEAP32[tmPtr >> 2] = date.getUTCSeconds()
    memoryViews.HEAP32[(tmPtr + 4) >> 2] = date.getUTCMinutes()
    memoryViews.HEAP32[(tmPtr + 8) >> 2] = date.getUTCHours()
    memoryViews.HEAP32[(tmPtr + 12) >> 2] = date.getUTCDate()
    memoryViews.HEAP32[(tmPtr + 16) >> 2] = date.getUTCMonth()
    memoryViews.HEAP32[(tmPtr + 20) >> 2] = date.getUTCFullYear() - 1900
    memoryViews.HEAP32[(tmPtr + 24) >> 2] = date.getUTCDay()
    const start = Date.UTC(date.getUTCFullYear(), 0, 1, 0, 0, 0, 0)
    const yday = ((date.getTime() - start) / (1e3 * 60 * 60 * 24)) | 0
    memoryViews.HEAP32[(tmPtr + 28) >> 2] = yday
  }
  ...
  function __localtime_js(time, tmPtr) {
    time = bigintToI53Checked(time)
    const date = new Date(time * 1e3)
    memoryViews.HEAP32[tmPtr >> 2] = date.getSeconds()
    memoryViews.HEAP32[(tmPtr + 4) >> 2] = date.getMinutes()
    memoryViews.HEAP32[(tmPtr + 8) >> 2] = date.getHours()
    memoryViews.HEAP32[(tmPtr + 12) >> 2] = date.getDate()
    memoryViews.HEAP32[(tmPtr + 16) >> 2] = date.getMonth()
    memoryViews.HEAP32[(tmPtr + 20) >> 2] = date.getFullYear() - 1900
    memoryViews.HEAP32[(tmPtr + 24) >> 2] = date.getDay()
    const yday =
      ydayFromDateCommon(
        date,
    ...
    memoryViews.HEAP32[(tmPtr + 28) >> 2] = yday
    memoryViews.HEAP32[(tmPtr + 36) >> 2] = -(date.getTimezoneOffset() * 60)
    ...
    memoryViews.HEAP32[(tmPtr + 32) >> 2] = dst
  }
  const __mktime_js = function (tmPtr) {
    const ret = (() => {
      const date = new Date(
        memoryViews.HEAP32[(tmPtr + 20) >> 2] + 1900,
        memoryViews.HEAP32[(tmPtr + 16) >> 2],
        memoryViews.HEAP32[(tmPtr + 12) >> 2],
        memoryViews.HEAP32[(tmPtr + 8) >> 2],
        memoryViews.HEAP32[(tmPtr + 4) >> 2],
        memoryViews.HEAP32[tmPtr >> 2],
        0,
      )
      const dst = memoryViews.HEAP32[(tmPtr + 32) >> 2]
      ...
      if (dst < 0) {
        memoryViews.HEAP32[(tmPtr + 32) >> 2] = Number(
      ...
      memoryViews.HEAP32[(tmPtr + 12) >> 2] = date.getDate()
      memoryViews.HEAP32[(tmPtr + 16) >> 2] = date.getMonth()
      memoryViews.HEAP32[(tmPtr + 20) >> 2] = date.getYear()
  ...
  function __mmap_js(len, prot, flags, fd, offset, allocated, addr) {
    ...
    try {
      ...
      const ptr = res.ptr
      memoryViews.HEAP32[allocated >> 2] = res.allocated
      memoryViews.HEAPU32[addr >> 2] = ptr
      return 0
    } catch (e) {
      if (typeof FS == 'undefined' || !(e.name === 'ErrnoError')) throw e
      return -e.errno
  ...
  const __tzset_js = (timezone, daylight, std_name, dst_name) => {
    ...
    memoryViews.HEAPU32[timezone >> 2] = stdTimezoneOffset * 60
    memoryViews.HEAP32[daylight >> 2] = Number(winterOffset != summerOffset)
    ...
    if (summerOffset < winterOffset) {
      stringToUTF8Array(winterName, memoryViews.HEAPU8, std_name, 17)
      stringToUTF8Array(summerName, memoryViews.HEAPU8, dst_name, 17)
    } else {
      stringToUTF8Array(winterName, memoryViews.HEAPU8, dst_name, 17)
      stringToUTF8Array(summerName, memoryViews.HEAPU8, std_name, 17)
    }
  }
  ...
  function _clock_time_get(clk_id, ignored_precision, ptime) {
    ...
    memoryViews.HEAP64[ptime >> 3] = BigInt(nsec)
    return 0
  }
  ...
  const _emscripten_resize_heap = (requestedSize) => {
    const oldSize = memoryViews.HEAPU8.length
    requestedSize >>>= 0
  ...
  const _environ_get = (__environ, environ_buf) => {
    ...
    getEnvStrings().forEach((string, i) => {
      ...
      memoryViews.HEAPU32[(__environ + i * 4) >> 2] = ptr
      for (let offset = 0; offset < string.length; ++offset) {
        memoryViews.HEAP8[ptr + offset] = string.charCodeAt(offset)
      }
      memoryViews.HEAP8[ptr + string.length] = 0
  ...
  const _environ_sizes_get = (penviron_count, penviron_buf_size) => {
    ...
    memoryViews.HEAPU32[penviron_count >> 2] = strings.length
    let bufSize = 0
    strings.forEach((string) => (bufSize += string.length + 1))
    memoryViews.HEAPU32[penviron_buf_size >> 2] = bufSize
    return 0
  }
      ...
      const type = stream.tty
        ...
        : FS.isDir(stream.mode)
          ...
          : FS.isLink(stream.mode)
            ? 7
            : 4
      memoryViews.HEAP8[pbuf] = type
      memoryViews.HEAP16[(pbuf + 2) >> 1] = flags
      memoryViews.HEAP64[(pbuf + 8) >> 3] = BigInt(rightsBase)
      memoryViews.HEAP64[(pbuf + 16) >> 3] = BigInt(rightsInheriting)
      return 0
    } catch (e) {
      if (typeof FS == 'undefined' || !(e.name === 'ErrnoError')) throw e
      return e.errno
  ...
  const doReadv = (stream, iov, iovcnt, offset) => {
    ...
    for (let i = 0; i < iovcnt; i++) {
      const ptr = memoryViews.HEAPU32[iov >> 2]
      const len = memoryViews.HEAPU32[(iov + 4) >> 2]
      iov += 8
      const curr = FS.read(stream, memoryViews.HEAP8, ptr, len, offset)
      if (curr < 0) return -1
      ret += curr
      if (curr < len) break
  ...
  function _fd_pread(fd, iov, iovcnt, offset, pnum) {
    ...
    try {
      ...
      memoryViews.HEAPU32[pnum >> 2] = num
      return 0
    } catch (e) {
      if (typeof FS == 'undefined' || !(e.name === 'ErrnoError')) throw e
      return e.errno
  ...
  const doWritev = (stream, iov, iovcnt, offset) => {
    ...
    for (let i = 0; i < iovcnt; i++) {
      ...
      const len = memoryViews.HEAPU32[(iov + 4) >> 2]
      iov += 8
      const curr = FS.write(stream, memoryViews.HEAP8, ptr, len, offset)
      if (curr < 0) return -1
      ret += curr
      if (curr < len) {
        break
  ...
  function _fd_seek(fd, offset, whence, newOffset) {
    ...
    try {
      ...
      memoryViews.HEAP64[newOffset >> 3] = BigInt(stream.position)
  ...
  const _getaddrinfo = (node, service, hint, out) => {
    let family = hint ? memoryViews.HEAP32[(hint + 4) >> 2] : 0
    let type = hint ? memoryViews.HEAP32[(hint + 8) >> 2] : 0
    let proto = hint ? memoryViews.HEAP32[(hint + 12) >> 2] : 0
    ...
    memoryViews.HEAP16[sockaddr >> 1] = 2
    memoryViews.HEAPU16[(sockaddr + 2) >> 1] = ((port & 255) << 8) | (port >> 8)
    memoryViews.HEAP32[(sockaddr + 4) >> 2] = address
    memoryViews.HEAP32[(sockaddr + 8) >> 2] = 0
    memoryViews.HEAP32[(sockaddr + 12) >> 2] = 0

    const addrinfo = _malloc(32)
    memoryViews.HEAP32[(addrinfo + 4) >> 2] = family
    memoryViews.HEAP32[(addrinfo + 8) >> 2] = type
    memoryViews.HEAP32[(addrinfo + 12) >> 2] = proto
    memoryViews.HEAP32[(addrinfo + 16) >> 2] = 16
    memoryViews.HEAPU32[(addrinfo + 20) >> 2] = sockaddr
    memoryViews.HEAPU32[(addrinfo + 24) >> 2] = 0
    memoryViews.HEAPU32[(addrinfo + 28) >> 2] = 0
    memoryViews.HEAPU32[out >> 2] = addrinfo
    return 0
  }
  const _getnameinfo = () => -6
  function _random_get(buffer, size) {
    try {
      crypto.getRandomValues(memoryViews.HEAPU8.subarray(buffer, buffer + size))
      return 0
    } catch (e) {
      if (typeof FS == 'undefined' || !(e.name === 'ErrnoError')) throw e
      return e.errno
  ...
  const ___cxa_can_catch = lazyWasmFunction('__cxa_can_catch')
  const findMatchingCatch = (args) => {
    ...
    const info = new ExceptionInfo(memoryViews, thrown)
  ...
  const ___cxa_throw = (ptr, type, destructor) => {
    const info = new ExceptionInfo(memoryViews, ptr)
  ...
  const FS_stdin_getChar = () => {
    if (!FS_stdin_getChar_buffer.length) {
      ...
      const buffer = new Array<number>(lengthBytesUTF8(result) + 1)
      const bytesWritten = stringToUTF8Array(result, buffer, 0, buffer.length)
      buffer.length = bytesWritten
      FS_stdin_getChar_buffer = buffer
    }
  ...
  const mmapAlloc = (size) => {
    ...
    if (ptr) memoryViews.HEAPU8.fill(0, ptr, ptr + size)
    return ptr
  }
  const MEMFS = {
    ops_table: null,
    ...
    stream_ops: {
      ...
      write(stream, buffer, offset, length, position, canOwn) {
        if (buffer.buffer === memoryViews.HEAP8.buffer) {
          canOwn = false
        }
        if (!length) return 0
      ...
      mmap(stream, length, position, prot, flags) {
        ...
        if (
          !(flags & 2) &&
          contents &&
          contents.buffer === memoryViews.HEAP8.buffer
        ) {
          allocated = false
          ...
          if (contents) {
            ...
            memoryViews.HEAP8.set(contents, ptr)
          }
        }
  ...
  const NODEFS = {
    ...
    stream_ops: {
      ...
      mmap(stream, length, position, prot, flags) {
        ...
        NODEFS.stream_ops.read(stream, memoryViews.HEAP8, ptr, length, position)
  ...
  const FS = {
    ...
    open(path, flags, mode = 438) {
      ...
      if (typeof flags == 'string') {
        const numericFlags = FS_FLAG_MODES[flags]
        if (typeof numericFlags === 'undefined') {
          throw new Error(`Unknown file open mode: ${flags}`)
        }
        flags = numericFlags
      }
      if (flags & 64) {
  ...
  const SYSCALLS = {
    ...
    doStat(func, path, buf) {
      ...
      memoryViews.HEAP32[buf >> 2] = stat.dev
      memoryViews.HEAP32[(buf + 4) >> 2] = stat.mode
      memoryViews.HEAPU32[(buf + 8) >> 2] = stat.nlink
      memoryViews.HEAP32[(buf + 12) >> 2] = stat.uid
      memoryViews.HEAP32[(buf + 16) >> 2] = stat.gid
      memoryViews.HEAP32[(buf + 20) >> 2] = stat.rdev
      memoryViews.HEAP64[(buf + 24) >> 3] = BigInt(stat.size)
      memoryViews.HEAP32[(buf + 32) >> 2] = 4096
      memoryViews.HEAP32[(buf + 36) >> 2] = stat.blocks
      const atime = stat.atime.getTime()
      const mtime = stat.mtime.getTime()
      const ctime = stat.ctime.getTime()
      memoryViews.HEAP64[(buf + 40) >> 3] = BigInt(Math.floor(atime / 1e3))
      memoryViews.HEAPU32[(buf + 48) >> 2] = (atime % 1e3) * 1e3 * 1e3
      memoryViews.HEAP64[(buf + 56) >> 3] = BigInt(Math.floor(mtime / 1e3))
      memoryViews.HEAPU32[(buf + 64) >> 2] = (mtime % 1e3) * 1e3 * 1e3
      memoryViews.HEAP64[(buf + 72) >> 3] = BigInt(Math.floor(ctime / 1e3))
      memoryViews.HEAPU32[(buf + 80) >> 2] = (ctime % 1e3) * 1e3 * 1e3
      memoryViews.HEAP64[(buf + 88) >> 3] = BigInt(stat.ino)
      return 0
    },
    doMsync(addr, stream, len, flags, offset) {
      ...
      const buffer = memoryViews.HEAPU8.slice(addr, addr + len)
  ...
  const ___syscall__newselect = function (
    ...
    try {
      let total = 0
      const srcReadLow = readfds ? memoryViews.HEAP32[readfds >> 2] : 0,
        srcReadHigh = readfds ? memoryViews.HEAP32[(readfds + 4) >> 2] : 0
      const srcWriteLow = writefds ? memoryViews.HEAP32[writefds >> 2] : 0,
        srcWriteHigh = writefds ? memoryViews.HEAP32[(writefds + 4) >> 2] : 0
      const srcExceptLow = exceptfds ? memoryViews.HEAP32[exceptfds >> 2] : 0,
        srcExceptHigh = exceptfds ? memoryViews.HEAP32[(exceptfds + 4) >> 2] : 0
      let dstReadLow = 0,
        dstReadHigh = 0
      let dstWriteLow = 0,
        dstWriteHigh = 0
      let dstExceptLow = 0,
        dstExceptHigh = 0
      const allLow =
        (readfds ? memoryViews.HEAP32[readfds >> 2] : 0) |
        (writefds ? memoryViews.HEAP32[writefds >> 2] : 0) |
        (exceptfds ? memoryViews.HEAP32[exceptfds >> 2] : 0)
      const allHigh =
        (readfds ? memoryViews.HEAP32[(readfds + 4) >> 2] : 0) |
        (writefds ? memoryViews.HEAP32[(writefds + 4) >> 2] : 0) |
        (exceptfds ? memoryViews.HEAP32[(exceptfds + 4) >> 2] : 0)
      ...
      for (let fd = 0; fd < nfds; fd++) {
        ...
        if (stream.stream_ops.poll) {
          ...
          if (timeout) {
            const tv_sec = readfds ? memoryViews.HEAP32[timeout >> 2] : 0,
              tv_usec = readfds ? memoryViews.HEAP32[(timeout + 4) >> 2] : 0
      ...
      if (readfds) {
        memoryViews.HEAP32[readfds >> 2] = dstReadLow
        memoryViews.HEAP32[(readfds + 4) >> 2] = dstReadHigh
      }
      if (writefds) {
        memoryViews.HEAP32[writefds >> 2] = dstWriteLow
        memoryViews.HEAP32[(writefds + 4) >> 2] = dstWriteHigh
      }
      if (exceptfds) {
        memoryViews.HEAP32[exceptfds >> 2] = dstExceptLow
        memoryViews.HEAP32[(exceptfds + 4) >> 2] = dstExceptHigh
      }
      return total
    } catch (e) {
      if (typeof FS == 'undefined' || !(e.name === 'ErrnoError')) throw e
  ...
  const syscallGetVarargI = () => {
    const ret = memoryViews.HEAP32[+SYSCALLS.varargs >> 2]
    SYSCALLS.varargs += 4
    return ret
  }
    ...
    try {
      ...
      switch (cmd) {
        ...
        case 12: {
          ...
          const offset = 0
          memoryViews.HEAP16[(arg + offset) >> 1] = 2
          return 0
        }
        case 13:
        case 14:
  ...
  function ___syscall_getcwd(buf, size) {
    try {
      ...
      stringToUTF8Array(cwd, memoryViews.HEAPU8, buf, size)
  ...
  function ___syscall_getdents64(fd, dirp, count) {
    try {
      ...
      for (; idx < endIdx; idx++) {
        ...
        memoryViews.HEAP64[(dirp + pos) >> 3] = BigInt(id)
        memoryViews.HEAP64[(dirp + pos + 8) >> 3] = BigInt(
          (idx + 1) * struct_size,
        )
        memoryViews.HEAP16[(dirp + pos + 16) >> 1] = 280
        memoryViews.HEAP8[dirp + pos + 18] = type
        stringToUTF8Array(name, memoryViews.HEAPU8, dirp + pos + 19, 256)
        pos += struct_size
      }
      ...
      switch (op) {
        ...
        case 21505: {
          ...
          if (stream.tty.ops.ioctl_tcgets) {
            ...
            memoryViews.HEAP32[argp >> 2] = termios.c_iflag || 0
            memoryViews.HEAP32[(argp + 4) >> 2] = termios.c_oflag || 0
            memoryViews.HEAP32[(argp + 8) >> 2] = termios.c_cflag || 0
            memoryViews.HEAP32[(argp + 12) >> 2] = termios.c_lflag || 0
            for (let i = 0; i < 32; i++) {
              memoryViews.HEAP8[argp + i + 17] = termios.c_cc[i] || 0
            }
            return 0
          }
          return 0
        ...
        case 21508: {
          ...
          if (stream.tty.ops.ioctl_tcsets) {
            let argp = syscallGetVarargP()
            const c_iflag = memoryViews.HEAP32[argp >> 2]
            const c_oflag = memoryViews.HEAP32[(argp + 4) >> 2]
            const c_cflag = memoryViews.HEAP32[(argp + 8) >> 2]
            const c_lflag = memoryViews.HEAP32[(argp + 12) >> 2]
            const c_cc = []
            for (let i = 0; i < 32; i++) {
              c_cc.push(memoryViews.HEAP8[argp + i + 17])
            }
        ...
        case 21523: {
          ...
          if (stream.tty.ops.ioctl_tiocgwinsz) {
            ...
            let argp = syscallGetVarargP()
            memoryViews.HEAP16[argp >> 1] = winsize[0]
            memoryViews.HEAP16[(argp + 2) >> 1] = winsize[1]
          }
          return 0
        }
        case 21524: {
  ...
  const wasmFunctionSignatures = new WeakMap<CallableFunction, string>()
  const wasmImports = {
  ...
  const registerWasmFunction = (
    importName: string,
    [fn, signature]: WasmFunction,
  ) => {
    wasmImports[importName] = fn
    wasmFunctionSignatures.set(fn, signature)
  }
  for (const [importName, wasmFunction] of Object.entries({
    __assert_fail: [___assert_fail, 'vppip'],
    __call_sighandler: [___call_sighandler, 'vpi'],
    __cxa_begin_catch: [___cxa_begin_catch, 'pp'],
    __cxa_current_primary_exception: [___cxa_current_primary_exception, 'p'],
    __cxa_end_catch: [___cxa_end_catch, 'v'],
    __cxa_find_matching_catch_2: [___cxa_find_matching_catch_2, 'p'],
    __cxa_find_matching_catch_3: [___cxa_find_matching_catch_3, 'pp'],
    __cxa_rethrow: [___cxa_rethrow, 'v'],
    __cxa_rethrow_primary_exception: [___cxa_rethrow_primary_exception, 'vp'],
    __cxa_throw: [___cxa_throw, 'vppp'],
    __cxa_uncaught_exceptions: [___cxa_uncaught_exceptions, 'i'],
    __resumeException: [___resumeException, 'vp'],
    __syscall__newselect: [___syscall__newselect, 'iipppp'],
    __syscall_accept4: [___syscall_accept4, 'iippiii'],
    __syscall_bind: [___syscall_bind, 'iippiii'],
    __syscall_chdir: [___syscall_chdir, 'ip'],
    __syscall_chmod: [___syscall_chmod, 'ipi'],
    __syscall_connect: [___syscall_connect, 'iippiii'],
    __syscall_dup: [___syscall_dup, 'ii'],
    __syscall_dup3: [___syscall_dup3, 'iiii'],
    __syscall_faccessat: [___syscall_faccessat, 'iipii'],
    __syscall_fadvise64: [___syscall_fadvise64, 'iijji'],
    __syscall_fallocate: [___syscall_fallocate, 'iiijj'],
    __syscall_fchmod: [___syscall_fchmod, 'iii'],
    __syscall_fchmodat2: [___syscall_fchmodat2, 'iipii'],
    __syscall_fchown32: [___syscall_fchown32, 'iiii'],
    __syscall_fchownat: [___syscall_fchownat, 'iipiii'],
    __syscall_fcntl64: [___syscall_fcntl64, 'iiip'],
    __syscall_fdatasync: [___syscall_fdatasync, 'ii'],
    __syscall_fstat64: [___syscall_fstat64, 'iip'],
    __syscall_ftruncate64: [___syscall_ftruncate64, 'iij'],
    __syscall_getcwd: [___syscall_getcwd, 'ipp'],
    __syscall_getdents64: [___syscall_getdents64, 'iipp'],
    __syscall_ioctl: [___syscall_ioctl, 'iiip'],
    __syscall_listen: [___syscall_listen, 'iiiiiii'],
    __syscall_lstat64: [___syscall_lstat64, 'ipp'],
    __syscall_mkdirat: [___syscall_mkdirat, 'iipi'],
    __syscall_newfstatat: [___syscall_newfstatat, 'iippi'],
    __syscall_openat: [___syscall_openat, 'iipip'],
    __syscall_pipe: [___syscall_pipe, 'ip'],
    __syscall_readlinkat: [___syscall_readlinkat, 'iippp'],
    __syscall_recvfrom: [___syscall_recvfrom, 'iippipp'],
    __syscall_renameat: [___syscall_renameat, 'iipip'],
    __syscall_rmdir: [___syscall_rmdir, 'ip'],
    __syscall_sendto: [___syscall_sendto, 'iippipp'],
    __syscall_socket: [___syscall_socket, 'iiiiiii'],
    __syscall_stat64: [___syscall_stat64, 'ipp'],
    __syscall_statfs64: [___syscall_statfs64, 'ippp'],
    __syscall_symlinkat: [___syscall_symlinkat, 'ipip'],
    __syscall_truncate64: [___syscall_truncate64, 'ipj'],
    __syscall_unlinkat: [___syscall_unlinkat, 'iipi'],
    __syscall_utimensat: [___syscall_utimensat, 'iippi'],
    _abort_js: [__abort_js, 'v'],
    _dlopen_js: [__dlopen_js, 'pp'],
    _dlsym_js: [__dlsym_js, 'pppp'],
    _emscripten_runtime_keepalive_clear: [
      __emscripten_runtime_keepalive_clear,
      'v',
    ],
    _emscripten_throw_longjmp: [__emscripten_throw_longjmp, 'v'],
    _gmtime_js: [__gmtime_js, 'vjp'],
    _localtime_js: [__localtime_js, 'vjp'],
    _mktime_js: [__mktime_js, 'jp'],
    _mmap_js: [__mmap_js, 'ipiiijpp'],
    _munmap_js: [__munmap_js, 'ippiiij'],
    _setitimer_js: [__setitimer_js, 'iid'],
    _tzset_js: [__tzset_js, 'vpppp'],
    clock_time_get: [_clock_time_get, 'iijp'],
    emscripten_date_now: [_emscripten_date_now, 'd'],
    emscripten_force_exit: [_emscripten_force_exit, 'vi'],
    emscripten_get_heap_max: [_emscripten_get_heap_max, 'p'],
    emscripten_get_now: [_emscripten_get_now, 'd'],
    emscripten_resize_heap: [_emscripten_resize_heap, 'ip'],
    environ_get: [_environ_get, 'ipp'],
    environ_sizes_get: [_environ_sizes_get, 'ipp'],
    exit: [_exit, 'vi'],
    fd_close: [_fd_close, 'ii'],
    fd_fdstat_get: [_fd_fdstat_get, 'iip'],
    fd_pread: [_fd_pread, 'iippjp'],
    fd_pwrite: [_fd_pwrite, 'iippjp'],
    fd_read: [_fd_read, 'iippp'],
    fd_seek: [_fd_seek, 'iijip'],
    fd_sync: [_fd_sync, 'ii'],
    fd_write: [_fd_write, 'iippp'],
    getaddrinfo: [_getaddrinfo, 'ipppp'],
    getnameinfo: [_getnameinfo, 'ipipipii'],
    proc_exit: [_proc_exit, 'vi'],
    random_get: [_random_get, 'ipp'],
    sched_yield: [_sched_yield, 'i'],
  } satisfies Record<string, WasmFunction>)) {
    registerWasmFunction(importName, wasmFunction)
  }
  if (runtimeOptions.filesystem.registerWasmPlugin) {
    registerWasmPlugin()
  }
  FS.createPreloadedFile = FS_createPreloadedFile
  FS.staticInit()
  if (loadPostgresPackage) {
  ...
  if (runtimeOptions.filesystem.initializeNodeFs) {
    NODEFS.staticInit()
  }
  for (const signature of [
    ...
    registerWasmFunction('invoke_' + signature, [
      createInvokeFunction(signature),
      signature,
    ])
  }

  const wasmInitialization = createWasm()
  const createPublicWasmFunctions = (wasmNames: string[]) =>
    Object.fromEntries(
      wasmNames.map((wasmName) => ['_' + wasmName, lazyWasmFunction(wasmName)]),
    )
  ...
  const ___dl_seterr = lazyWasmFunction('__dl_seterr')
  ...
  const __emscripten_tempret_set = lazyWasmFunction('_emscripten_tempret_set')
  const __emscripten_tempret_get = lazyWasmFunction('_emscripten_tempret_get')
  ...
  const callMain = createCallMain({
    ...
    stackAlloc,
    getHeapU32: () => memoryViews.HEAPU32,
    stringToUTF8OnStack,
    exitJS,
    handleException,
  })
  const runtimePublicApi =
    runtimeOptions.publicApi === 'initdb'
      ? {
          PROXYFS,
          ...createPublicWasmFunctions(['pclose']),
        }
      : {
...
export default createEmscriptenRuntime
